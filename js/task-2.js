...
const createCard = item => {
  const gallerryItemEl = document.createElement('li');
  gallerryItemEl.classList.add('gallery-item');

  const galleryItemImg = document.createElement('img');

  galleryItemImg.src = item.url;
  galleryItemImg.alt = item.alt;
  galleryItemImg.style.width = '300px';

  

  gallerryItemEl.append(galleryItemImg);
  return gallerryItemEl;
};

const galleryArr = images.map(img => createCard(img));

refs.galleryEl.append(...galleryArr);
